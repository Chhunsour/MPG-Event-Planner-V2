...
const schema = z.object({
  ...
  preferred_contact_method: z.string().trim().max(50).optional().default('telegram'),
  event_type: z.string().trim().max(100).optional().default('other'),
  event_date: z.string().trim().max(30).optional(),
  event_location: z.string().trim().max(255).optional().default('Not specified'),
...
export async function submitQuotation(_previous: QuotationState, formData: FormData): Promise<QuotationState> {
  ...
  const { data, error } = await supabase.from('quotations').insert({
    ...
    email,
    preferred_contact_method: parsed.data.preferred_contact_method || 'telegram',
    event_type: parsed.data.event_type || 'other',
